...
class PCN(nn.Module):
    def __init__(self, num_pred, encoder_channel, loss):
        super().__init__()
    ...
    def get_loss(self, coarse, fine, gt):
        ...
        loss_fine = self.loss_func(fine, gt)
        return dict(loss_coarse=loss_coarse, loss_fine=loss_fine)

    def forward(self, gt, xyz, device):
        bs , n , _ = xyz.shape
        # encoder
        ...
        if self.training:
            return self.get_loss(coarse.contiguous(), fine.transpose(2,1).contiguous(), gt)
        return dict(coarse_pred=coarse.contiguous(), pred=fine.transpose(2,1).contiguous(), ref=gt, input=xyz)
