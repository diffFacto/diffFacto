...
class LatentDiffusionDrift(Module):
    #TODO
    def __init__(
        self, 
        net, 
        ...
        self.posterior_mean_coef3 = (
            1.0 + ((self.alphas_cumprod - 1.) * ( np.sqrt(self.alphas_cumprod_prev) + np.sqrt(alphas)))
            / (1.0 - self.alphas_cumprod)
        )
        
        
    def q_mean_variance(self, x_start, t, prior):
        """
        ...
        """
        mean = (
            extract_into_tensor(self.sqrt_alphas_cumprod, t, x_start.shape) * (x_start - prior) + prior
        )
    ...
    def q_sample(self, x_start, t, prior, noise=None):
        """
    ...
    def q_posterior_mean(self, x_start, x_t, t, prior):
        """
        ...
        posterior_mean = (
            ...
            + extract_into_tensor(self.posterior_mean_coef3, t, x_t.shape) * prior
        )
        assert (
    ...
    def q_posterior_mean_variance(self, x_start, x_t, t, prior):
        """
        Compute the mean and variance of the diffusion posterior:
            q(x_{t-1} | x_t, x_0)
        """
        assert x_start.shape == x_t.shape == prior.shape
    
        
        posterior_mean = self.q_posterior_mean(x_start, x_t, t, prior)
        posterior_variance, posterior_log_variance_clipped = self.q_posterior_variance(x_start, x_t, t)
        return posterior_mean, posterior_variance, posterior_log_variance_clipped
    
    
    def p_mean_variance(
        self, x, t, prior, code, part_indicator=None
    ):
        """
        ...
        B, C = x.shape[:2]
        assert t.shape == (B,)
        model_output_cond = self.model(x,  self._scale_timesteps(t), prior, code, part_indicator=part_indicator)

        model_output_uncond = self.model(x, self._scale_timesteps(t), torch.zeros_like(prior).cuda(), torch.zeros_like(code).cuda(), part_indicator=part_indicator)
        ...
        if self.model_mean_type == ModelMeanType.PREVIOUS_X:
            pred_xstart = process_xstart(
                self._predict_xstart_from_xprev(x_t=x, t=t, prior=prior, xprev=model_output)
            )
            ...
            if self.model_mean_type == ModelMeanType.START_X:
                ...
                pred_xstart = process_xstart(
                    self._predict_xstart_from_eps(x_t=x, t=t, prior=prior, eps=model_output)
                )
            model_mean, _, _ = self.q_posterior_mean_variance(
                x_start=pred_xstart, x_t=x, t=t, prior=prior
            )
        else:
    ...
    def _predict_xstart_from_eps(self, x_t, t, prior, eps):
        assert x_t.shape == eps.shape == prior.shape
        
        return (extract_into_tensor(self.sqrt_recip_alphas_cumprod, t, x_t.shape) * (x_t - prior) + prior
                - extract_into_tensor(self.sqrt_recipm1_alphas_cumprod, t, eps.shape) * eps
                )

    def _predict_xstart_from_xprev(self, x_t, t, prior, xprev):
        assert x_t.shape == xprev.shape == prior.shape
    ...
    def _predict_eps_from_xstart(self, x_t, t, prior, pred_xstart):
        return (
            extract_into_tensor(self.sqrt_recip_alphas_cumprod, t, x_t.shape) * (x_t - prior)
            - pred_xstart
    ...
    def p_sample_loop_progressive(
        ...
        if noise is not None:
            ...
            pcd = torch.randn(*shape, device=device) + prior
    ...
    def q_sample_loop(
        ...
        return final 
    def q_sample_loop_progressive(self, gt_pcd, prior, noise=None, device=None, progress=False):
        if device is None:
            device = next(self.model.parameters()).device
        indices = list(range(self.num_timesteps))[1:]
        if noise is None:
        ...
        for i in indices:
            ...
            with torch.no_grad():
                yield i, self.q_sample(gt_pcd, t, prior, noise=noise)
    ...
    def training_losses(self, x_start, t, prior, code, part_indicator=None, noise=None):
        """
        ...
        if noise is None:
            noise = torch.randn_like(x_start)
        x_t = self.q_sample(x_start, t, torch.zeros_like(prior), noise=noise)
        # x_t = self.q_sample(x_start, t, prior, noise=noise)

        terms = {}

        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            model_output = self.model(x_t, self._scale_timesteps(t), prior, code, part_indicator=part_indicator)
            target = {
