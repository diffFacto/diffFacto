...
if __name__ == '__main__':
        ...
        for j in range(4):
            ...
            if torch.any(idx):
                ...
                if points.shape[0] > 100:
                    ...
                    part_max = torch.quantile(points, thresh, dim=0, keepdim=True) 
                    part_min =  torch.quantile(points, 1 - thresh, dim=0, keepdim=True)
                    # part_std = (torch.quantile(points, thresh, dim=0, keepdim=True) - torch.quantile(points, 1 - thresh, dim=0, keepdim=True)) / 2.0
                    one_pred_params.append(torch.rand(args.num_points_sample, 3).cuda() * (part_max - part_min) + part_min)
        ...
        for j in range(4):
            ...
            if args.cat != "Car":
                ...
                if ref_points.abs().sum() != 0:
                    ref_part_max = torch.quantile(ref_points, thresh, dim=0, keepdim=True) 
                    ref_part_min =  torch.quantile(ref_points, 1 - thresh, dim=0, keepdim=True)
                    one_ref_params.append(torch.rand(args.num_points_sample, 3).cuda() * (ref_part_max - ref_part_min) + ref_part_min)
            else:
    ...
    pickle.dump(dict(pred=ref_params.cpu().numpy(), pred_ref=ref.cpu().numpy()), open("/orion/u/w4756677/diffusion/anchorDIff/weights/out_ref.pkl", 'wb'))
    pickle.dump(dict(pred=pred_params.cpu().numpy(), pred_ref=pred.cpu().numpy()), open("/orion/u/w4756677/diffusion/anchorDIff/weights/out.pkl", 'wb'))
