...
class NoiseOptimizer:
    def __init__(self, device, args):
        ...
        self.dataset = _ShapeNetSegParts(**cfg.dataset)
        self.loader = DataLoader(self.dataset, batch_size=args.batch_size, shuffle=False, num_workers=0)
    ...
    def optimize(self, pcds, batch_idx, start_time):
        if self.logger is not None:
            self.logger.print_log("Start running")
        noise, _ = self.model.encoder.sample_noise(pcds, 'cuda', 1)
        ...
        for it in range(self.max_iter):
            optimizer.zero_grad()
            losses = self.model.optimize_latent(pcds, z, device=self.local_rank)
...
def main():
    ...
    runner = NoiseOptimizer(device, args)
